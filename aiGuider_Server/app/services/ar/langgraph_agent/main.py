...
from .config.error_codes import ConfigError, ErrorCodes, ModelError
...
class ARGuideAgent:
    ...
    def __init__(self, model_name: str):
        ...
        try:
            ...
            if CHECKPOINT_AVAILABLE:
                try:
                    # 尝试从环境变量获取检查点目录，如果没有设置，则使用默认值 "./checkpoints"
                    checkpoint_dir = os.environ.get("CHECKPOINT_DIR", "./checkpoints")
                    # 使用获取到的目录初始化 LocalStateCheckpointManager
    ...
    def _initialize_model(self, model_name: str):
        ...
        Raises:
            ModelError: 当模型初始化失败时抛出
            ConfigError: 当模型类型不支持时抛出
        """
        # 生成错误调试指纹
        ...
        try:
            ...
            #     return get_gemini_model(self.config)
                ...
                # 使用新的错误处理机制
                error_message = f"不支持的模型类型: {model_name}"
                logger.error(f"{error_message} [错误码: {ErrorCodes.MODEL_TYPE_NOT_SUPPORTED}, ID: {error_id}]")
                raise ModelError(
                    message=error_message,
                    error_code=ErrorCodes.MODEL_TYPE_NOT_SUPPORTED,
                    error_id=error_id
                )
            ...
            logger.error(f"{error_message} [错误码: {ErrorCodes.MODEL_INIT_FAILED}, ID: {error_id}]", exc_info=True)
            raise ModelError(
                message=error_message,
...
def get_agent_instance(model_name: str) -> ARGuideAgent:
    """
    获取ARGuideAgent单例实例
